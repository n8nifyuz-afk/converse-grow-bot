...
export const PricingModal: React.FC<PricingModalProps> = ({ open, onOpenChange }) => {
  ...
  const modalContent = (
    <div className="light flex flex-col md:flex-row h-full bg-white md:bg-transparent md:overflow-hidden relative">
            ...
            {/* Right Panel - Pricing */}
            <div className="w-full md:w-5/12 p-4 sm:p-5 md:p-5 pb-5 sm:pb-6 flex flex-col bg-gradient-to-br from-white to-zinc-50/50 justify-between min-h-0 md:overflow-visible">
  ...
  return (
    <>
      {isMobile ? (
        <Drawer open={open} onOpenChange={onOpenChange} snapPoints={[1, 0.5]} dismissible={true}>
          <DrawerContent className="h-screen bg-gradient-to-br from-white via-zinc-50/50 to-white border-t border-zinc-300 overflow-y-auto">
            {modalContent}
          </DrawerContent>
        </Drawer>
      ) : (
