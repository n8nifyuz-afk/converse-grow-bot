    ...
    <div className="light flex flex-col md:flex-row h-full">
            ...
            <div className="hidden md:flex md:w-7/12 bg-gradient-to-br from-white via-zinc-50/50 to-white p-3 sm:p-4 md:p-4 lg:p-8 border-r border-zinc-200 flex-col relative">
              ...
              <div className="flex-1 flex flex-col min-h-0 relative z-10">
                {/* Header Row */}
                <div className="grid grid-cols-[2fr,0.8fr,2.5fr] gap-3 md:gap-4 pb-3 md:pb-4 border-b border-zinc-300 mb-2 flex-shrink-0">
                  <div className="text-sm md:text-base font-bold text-zinc-900">{t('pricingModal.modelFeature')}</div>
                  <div className="text-sm md:text-base font-bold text-center text-zinc-900">{t('pricingModal.included')}</div>
                  <div className="text-sm md:text-base font-bold text-zinc-900">{t('pricingModal.descriptionNotes')}</div>
                </div>
                    ...
                    <div key={index} className="grid grid-cols-[2fr,0.8fr,2.5fr] gap-3 md:gap-4 py-2 md:py-2.5 px-2 md:px-3 rounded-lg hover:bg-zinc-100 transition-all duration-200 backdrop-blur-sm border border-transparent hover:border-zinc-300">
                      ...
                      <div className="flex justify-center items-center">
                        {feature.name === 'Chat on WhatsApp' ? (
                          <span className="text-xs text-zinc-500">{t('pricingModal.comingSoon')}</span>
                        ) : feature.included ? (
                          ...
                          <div className="w-5 h-5 md:w-6 md:h-6 rounded-full bg-red-50 flex items-center justify-center">
                            <X className="w-3 h-3 md:w-4 md:h-4 text-red-500" />
                          </div>
                        )}
                      </div>
                      <div className="text-xs md:text-sm text-zinc-700 flex items-center leading-tight">
                        {feature.name === 'Priority Support' && selectedPlan === 'ultra' ? (
                          <>
                            <Check className="w-3 h-3 md:w-4 md:h-4 text-emerald-600 mr-1" />
            ...
            {/* Right Panel - Pricing */}
            <div className="w-full md:w-5/12 p-3 sm:p-4 md:p-5 pb-4 sm:pb-6 flex flex-col bg-gradient-to-br from-white to-zinc-50/50 justify-between min-h-0 gap-2 sm:gap-0">
              <div className="mb-1.5 sm:mb-2.5 flex-shrink-0">
              ...
              {/* Plan Tabs */}
              <Tabs value={selectedPlan} onValueChange={(v) => setSelectedPlan(v as 'pro' | 'ultra')} className="mb-1.5 sm:mb-2.5 flex-shrink-0">
              ...
              <div className="space-y-1.5 sm:space-y-2 mb-1.5 sm:mb-2.5 flex flex-col flex-1 overflow-y-auto md:flex-initial md:block md:overflow-visible">
              ...
              <Button 
                ...
                disabled={isLoading}
                className="w-full h-11 sm:h-11 bg-black hover:bg-zinc-800 text-white font-bold text-sm sm:text-sm shadow-lg hover:shadow-black/30 transition-all rounded-lg border-0 flex-shrink-0 mt-auto mb-2 sm:mb-4"
              >
                {isLoading ? (
                  <>
              ...
              {/* Footer */}
              <div className="space-y-1.5 sm:space-y-1 flex-shrink-0">
                {/* Pay Safe & Secure */}
