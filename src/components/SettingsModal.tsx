...
export default function SettingsModal({ open, onOpenChange }: SettingsModalProps) {
  ...
  const { usageLimits, loading: limitsLoading, checkLimits } = useUsageLimits();
                ...
                <Card className="shadow-sm border-border/40">
                  <CardContent className="p-4 md:p-6 space-y-3">
                    ...
                    {linkedIdentities.map((identity) => {
                      ...
                      const isApple = identity.provider === 'apple';
                      const isMicrosoft = identity.provider === 'azure';
                      const canUnlink = linkedIdentities.length > 1;
                      
                      // Get email from identity data if available
                      const identityEmail = identity.identity_data?.email || identity.email;

                      return (
                        <div key={identity.id} className="p-3 bg-muted/30 rounded-lg border">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3 min-w-0">
                              <div className="p-2 bg-primary/10 rounded-lg flex-shrink-0">
                                {isPhone && <Mail className="h-4 w-4 text-primary" />}
                              ...
                              <div className="min-w-0">
                                <p className="font-medium text-sm">
                                  ...
                                  {isGoogle && (identityEmail ? `Google (${identityEmail})` : 'Google Account')}
                                  {isApple && (identityEmail ? `Apple (${identityEmail})` : 'Apple ID')}
                                  {isMicrosoft && (identityEmail ? `Microsoft (${identityEmail})` : 'Microsoft Account')}
                                </p>
                                <p className="text-xs text-muted-foreground truncate">
                                  {isPhone && (identity.identity_data?.phone || userProfile?.phone_number)}
                                  {isEmail && (identity.identity_data?.email || 'Email login')}
                                  {(isGoogle || isApple || isMicrosoft) && t('settings.oauthLoginMethod')}
                                </p>
                              </div>
                            </div>
                    ...
                    <div className="space-y-2">
                      ...
                      */}
                      ...
                      {(!linkedIdentities.some(id => id.provider === 'google') ||
                        !linkedIdentities.some(id => id.provider === 'apple') ||
