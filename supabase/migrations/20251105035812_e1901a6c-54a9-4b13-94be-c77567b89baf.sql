-- Fix handle_new_user() trigger to prevent duplicate webhooks for phone signups
-- and properly track webhook sending status
...
DECLARE
  ...
  v_trace_data TEXT[];
  v_trace_line TEXT;
  v_should_send_webhook BOOLEAN := false;
BEGIN
  ...
  -- CRITICAL CHANGE: Only send webhook for email/password signups on new user creation
  -- Phone signups: webhook sent after profile completion via send-profile-complete-webhook
  -- OAuth signups: webhook sent after GCLID sync
  IF v_is_new_signup AND v_signup_method = 'email' AND NEW.email IS NOT NULL THEN
    v_should_send_webhook := true;
  ...
  -- Send webhook only for email signups
