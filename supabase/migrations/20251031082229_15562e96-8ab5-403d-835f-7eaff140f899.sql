-- Fix handle_new_user trigger to preserve manually entered display names
-- The issue: When users complete their profile after phone signup, the display_name
-- gets overwritten with 'User XXXX' on subsequent logins because the trigger
-- regenerates the auto-name and applies it via ON CONFLICT.
...
BEGIN
  ...
  ON CONFLICT (user_id) 
  DO UPDATE SET
    -- FIXED: Preserve existing display_name if it doesn't match auto-generated pattern
    -- This prevents overwriting manually entered names like "John Doe" with "User 9575"
    display_name = CASE
      -- If existing display_name doesn't match 'User XXXX' pattern, keep it
      ...
      -- Otherwise, use the new value (for OAuth updates or initial setup)
