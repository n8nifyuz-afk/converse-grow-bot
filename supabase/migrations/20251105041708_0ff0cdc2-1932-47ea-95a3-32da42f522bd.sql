-- Add username validation to prevent webhook for incomplete profiles
-- Phone webhooks should ONLY be sent by send-profile-complete-webhook with complete name
...
BEGIN
  ...
  -- Check if profile is complete (not default "User XXXX" format)
  v_has_complete_profile := v_display_name IS NOT NULL 
    AND v_display_name !~ '^User [0-9]+$' 
    AND length(v_display_name) > 5;
  
  v_signup_method := CASE
    WHEN NEW.phone IS NOT NULL AND (NEW.email IS NULL OR NEW.email ~ '^\+?[0-9]+$') THEN 'phone'
    WHEN v_phone IS NOT NULL AND (NEW.email IS NULL OR NEW.email ~ '^\+?[0-9]+$') THEN 'phone'
  ...
  VALUES (
    NEW.id,
    v_display_name,
    CASE WHEN NEW.email ~ '^\+?[0-9]+$' THEN NULL ELSE NEW.email END,
    v_signup_method,
    v_avatar_url,
    v_ip_address,
    v_country,
  ...
  DO UPDATE SET
    ...
    END,
    email = CASE WHEN EXCLUDED.email !~ '^\+?[0-9]+$' THEN COALESCE(EXCLUDED.email, profiles.email) ELSE profiles.email END,
  ...
  -- WEBHOOK LOGIC: Only send for email signups WITH complete profile
  -- Never send for incomplete profiles (default "User XXXX" format)
  -- Phone: webhook sent ONLY by send-profile-complete-webhook
  -- OAuth: webhook sent after GCLID sync
  IF v_is_new_signup AND v_signup_method = 'email' AND v_has_complete_profile THEN
    RAISE NOTICE '📤 Sending subscriber webhook for NEW email user % with complete profile', NEW.id;
  ...
  ELSE
    RAISE NOTICE '⏭️ SKIPPING webhook for user % (method: %, complete_profile: %) - webhook will be sent later', 
      NEW.id, v_signup_method, v_has_complete_profile;
  END IF;
  
  IF v_is_new_signup AND NEW.email IS NOT NULL AND NEW.email !~ '^\+?[0-9]+$' THEN
    PERFORM
      net.http_post(
  ...
  IF v_is_new_signup AND v_signup_method != 'phone' AND NEW.email IS NOT NULL AND NEW.email !~ '^\+?[0-9]+$' THEN
    PERFORM
      net.http_post(
