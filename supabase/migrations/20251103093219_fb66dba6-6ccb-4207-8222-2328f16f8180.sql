-- Update handle_new_user to skip webhook for OAuth signups (handled by frontend after GCLID sync)
-- Only send webhook immediately for email/phone signups where all data is available
...
BEGIN
  ...
  -- CRITICAL: Only send webhook for email/phone signups, NOT OAuth
  -- OAuth webhook is sent by frontend after GCLID data is synced from localStorage
  IF v_is_new_signup AND NOT v_is_oauth THEN
    RAISE NOTICE '📤 Sending subscriber webhook for NEW email/phone user %', NEW.id;
    ...
    RAISE NOTICE '✅ Subscriber webhook sent for email/phone user %', NEW.id;
