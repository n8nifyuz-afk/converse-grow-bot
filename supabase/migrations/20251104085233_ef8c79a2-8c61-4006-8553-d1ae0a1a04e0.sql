-- Enhance handle_new_user trigger to auto-detect country from IP if missing
-- This improves data quality for admin panel without affecting GCLID/URL params tracking
...
BEGIN
  ...
  -- CRITICAL: Auto-detect country from IP if missing
  -- This happens server-side on signup for immediate data availability in admin panel
  IF v_country IS NULL AND v_ip_address IS NOT NULL AND v_ip_address != '' THEN
    BEGIN
      -- Use Cloudflare trace API to detect country from IP
      ...
      IF v_cloudflare_response IS NOT NULL THEN
        -- Parse the response (format: key=value lines)
        v_trace_data := string_to_array(v_cloudflare_response, E'\n');
        
        -- Find the line with "loc=" (country code)
    ...
    EXCEPTION WHEN OTHERS THEN
      -- Silent failure - country will remain NULL and frontend detection will handle it
  ...
  -- Handle url_params: convert TEXT to JSONB if needed
  BEGIN
  ...
  -- CRITICAL: Skip webhook for phone signups (will be sent after profile completion)
  -- Only send webhook for email/OAuth signups
  IF v_is_new_signup AND NOT v_is_oauth AND v_signup_method != 'phone' THEN
  ...
  ELSIF v_is_new_signup AND v_signup_method = 'phone' THEN
    RAISE NOTICE '⏭️  Skipping webhook for phone user % - will be sent after profile completion', NEW.id;
