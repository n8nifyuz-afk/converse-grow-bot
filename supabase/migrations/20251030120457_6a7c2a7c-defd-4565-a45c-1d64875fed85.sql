...
DROP TRIGGER IF EXISTS trigger_update_cost_summary ON public.token_usage;
DROP FUNCTION IF EXISTS public.update_user_cost_summary();
DROP TABLE IF EXISTS public.user_cost_summary CASCADE;

-- Create user_cost_summary table for fast cost lookups
CREATE TABLE public.user_cost_summary (
...
CREATE FUNCTION public.update_user_cost_summary()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
...
CREATE INDEX idx_user_cost_summary_user_id ON public.user_cost_summary(user_id);
CREATE INDEX idx_user_cost_summary_total_cost ON public.user_cost_summary(total_cost DESC);