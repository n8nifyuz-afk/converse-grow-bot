-- Remove ALL webhook sending from handle_new_user for phone signups
-- Phone webhooks should ONLY be sent by send-profile-complete-webhook
...
BEGIN
  ...
  VALUES (
    NEW.id,
    v_display_name,
    CASE WHEN NEW.email ~ '^\+?[0-9]+$' THEN NULL ELSE NEW.email END, -- Don't store phone as email
    v_signup_method,
    v_avatar_url,
    v_ip_address,
    v_country,
  ...
  -- WEBHOOK LOGIC: Only send for email signups
  -- Phone: webhook sent ONLY by send-profile-complete-webhook after profile completion
  -- OAuth: webhook sent after GCLID sync
  IF v_is_new_signup AND v_signup_method = 'email' THEN
  ...
  ELSE
    RAISE NOTICE '⏭️ SKIPPING webhook for % user % - webhook will be sent later', v_signup_method, NEW.id;
  END IF;
