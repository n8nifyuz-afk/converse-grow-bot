  ...
  try {
    ...
    switch (event.type) {
      ...
      case "invoice.payment_succeeded": {
        ...
        // Check if this is a trial conversion (first paid invoice after trial)
        if (invoice.subscription) {
          try {
            const subscription = await stripe.subscriptions.retrieve(invoice.subscription as string);
            const isTrialConversion = subscription.metadata?.is_trial === 'true' 
              && (invoice.billing_reason === 'subscription_cycle' || invoice.billing_reason === 'subscription_create')
              && invoice.amount_paid > 0;

            if (isTrialConversion) {
              logStep("Trial conversion detected", { 
                userId: user.id,
                subscriptionId: subscription.id,
                invoiceId: invoice.id
              });

              // Track conversion in trial_conversions table (only insert if not exists)
              const { error: conversionInsertError } = await supabaseClient
                .from('trial_conversions')
                .insert({
                  user_id: user.id,
                  trial_subscription_id: subscription.id,
                  trial_product_id: subscription.metadata?.trial_product_id || '',
                  target_plan: subscription.metadata?.plan || 'pro',
                  paid_subscription_id: subscription.id,
                  converted_at: new Date().toISOString()
                });

              // Ignore duplicate key errors (user already converted)
              if (conversionInsertError && !conversionInsertError.message.includes('duplicate key')) {
                logStep("WARNING: Failed to insert trial conversion", { 
                  error: conversionInsertError.message,
                  userId: user.id
                });
              } else {
                logStep("Trial conversion tracked", { userId: user.id });
              }
            }
          } catch (conversionError) {
            logStep("ERROR: Failed to track trial conversion", { 
              error: conversionError instanceof Error ? conversionError.message : String(conversionError),
              userId: user.id
            });
            // Continue processing even if conversion tracking fails
          }
        }
